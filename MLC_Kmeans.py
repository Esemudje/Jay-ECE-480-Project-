...
def read_mfccs_for_block(lines, block_start, block_size):
    ...
    for i in range(block_start, block_start + block_size):
        ...
        if frame:  # Ignore blank lines
            parts = frame.split()
            if len(parts) >= 3:  # Ensure there are at least 3 elements
                mfccOne, mfccTwo, mfccThree = parts[:3]
                mfccOneList.append(float(mfccOne))
                mfccTwoList.append(float(mfccTwo))
                mfccThreeList.append(float(mfccThree))

    return np.column_stack((mfccOneList, mfccTwoList, mfccThreeList))
...
for digit in range(10):
    ...
    digit_data = np.vstack([
        read_mfccs_for_block(all_lines, 
                             get_block_start_for_digit(digit) + sum(block_counts[digit * 660:(digit * 660) + i]), 
                             block_counts[(digit * 660) + i])
        for i in range(660)
    ])
    kmeans = KMeans(n_clusters=4, random_state=0).fit(digit_data)
    gmm = GaussianMixture(n_components=4, covariance_type='full', random_state=0, means_init=kmeans.cluster_centers_)
    gmm.fit(digit_data)
    gmms[digit] = gmm
