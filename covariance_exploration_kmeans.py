...
for digit in range(10):
    with open(filename, "r") as file:
        ...
        kmeans = KMeans(n_clusters=2, random_state=50).fit(data)
        ...
        # Enforce tied diagonal covariance
        # Extract the shared covariance matrix and convert it to diagonal
        shared_cov = gmm.covariances_
        tied_diag_cov = np.diag(np.diag(shared_cov))  # Keep only the diagonal part

        # Manually assign the tied diagonal covariance
        gmm.covariances_ = tied_diag_cov
        ...
        #plt.savefig(f"kmeans_tied_diagonal_{digit}.png")
        plt.show()
